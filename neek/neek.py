...
# ------------------ 5. SEND WHATSAPP TEXT ------------------
def send_whatsapp_by_name(contact_name, message):
    ...
    with sr.Microphone() as source:
        ...
        while True:
            ...
            if 'hello' in query or 'hi' in query:
                ...
                speak("Aapke system ka khayal rakh raha hoon.")

            elif 'khana khaya' in query:
                speak("Main toh data aur bijli se chalta hoon.")

            elif 'battery' in query:
                battery = psutil.sensors_battery()
                speak(f"Battery is at {battery.percent} percent")

            # ✅ WHATSAPP TEXT (FIXED FLOW)
            elif 'send whatsapp message' in query or "send a message" in query or "send  message" in query or "sending a message" in query:
                speak("Tell me the contact name")
                name = listen_to_user(r, source, "Listening for contact name...")

                if name == "none":
                    speak("I did not get the contact name. Please try again.")
                    continue

                speak("What is the message?")
                msg = listen_to_user(r, source, "Listening for message...")

                if msg == "none":
                    speak("I did not get the message. Cancelling.")
                    continue
            ...
            # System
                ...
                speak("youtube opened ho raha hy bro!")
                webbrowser.open("https://youtube.com")

            elif 'open google' in query:
                speak("google opend ho raha hy bro!")
